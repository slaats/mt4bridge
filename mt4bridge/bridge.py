...
class MT4Bridge:
    ...
    def __init__(self, address="tcp://localhost:5555"):
        """
        Initializes the MT4Bridge instance by setting up a ZeroMQ REQ socket connected to the specified address.

        Args:
            address (str, optional): The ZeroMQ address to connect to. Defaults to "tcp://localhost:5555".
    ...
    def send_request(self, request_str):
        ...
        Args:
            request_str (str): The formatted request string to send (e.g., "HIST:EURUSD:H1:100").
    ...
    def get_historical_data(self, symbol="EURUSD", timeframe="H1", bars=10):
        """
        Requests historical candlestick/bar data for a specific symbol and timeframe.

        Args:
            symbol (str, optional): The trading symbol (e.g., "EURUSD", "XAUUSD.a"). Defaults to "EURUSD".
            timeframe (str, optional): The timeframe for the bars (e.g., "M1", "H1", "D1"). Defaults to "H1".
            bars (int, optional): The number of historical bars to retrieve. Defaults to 10.

        Returns:
            list[dict] or None: A list of dictionaries containing bar data, or None if an error occurs.
        """
    ...
    def get_current_tick(self, symbol: str):
        """
        Requests the current tick information (Bid, Ask, Last price, Volume, Time) for a specific symbol.

        Args:
            symbol (str): The trading symbol (e.g., "EURUSD", "XAUUSD.a").

        Returns:
            dict or None: A dictionary containing current tick information, or None if an error occurs.
        """
    ...
    def get_all_timeframes(self, symbol: str):
        """
        Requests the latest candlestick/bar data for all supported timeframes for a specific symbol.

        Args:
            symbol (str): The trading symbol (e.g., "EURUSD", "XAUUSD.a").

        Returns:
            list[dict] or None: A list of dictionaries, each containing the latest bar data for a timeframe,
    ...
    def get_indicator(self, symbol, timeframe, indicator_name, params, bars):
        """
        Requests indicator values for a specific symbol, timeframe, and indicator configuration.

        Args:
            symbol (str): The trading symbol (e.g., "EURUSD", "XAUUSD.a").
            timeframe (str): The timeframe for the indicator (e.g., "M1", "H1", "D1").
            indicator_name (str): The name of the indicator (e.g., "MA" for Moving Average).
            params (str): A comma-separated string of parameters required by the indicator
                          (e.g., "14,0,1,0" for MA period=14, shift=0, method=1, applied_price=0).
            bars (int): The number of indicator values to retrieve.

        Returns:
            list[dict] or None: A list of dictionaries containing indicator values per bar, or None if an error occurs.
        """
    ...
    def _parse_response(self, reply):
        ...
        Returns:
            dict or list[dict] or None: The parsed JSON data as a dictionary or list of dictionaries,
                                        or None if parsing fails or an error is present.
        """
        if not reply:
    ...
    def close(self):
        ...
        It's good practice to call this method when the bridge is no longer needed to free up resources.
        """
        self.socket.close()
        self.context.term()
